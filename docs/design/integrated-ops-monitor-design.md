...
- **색상 규약**: SM37 / ST22 / SXI 각각 고정 색상을 부여하여 차트·범례·ALV 행색 일관성 유지.
  - 차트는 IGS `SimpleChartData` + `Series customizing="Series1"` 과
    `Values/Series/Color|LineColor = RGB(r,g,b)` 로 지정(헥스/`Point/RGB` 는 미반영).
  - SM37: ALV `C400` / 차트 `RGB(198,40,40)` (적)
  - ST22: ALV `C600` / 차트 `RGB(249,168,37)` (황)
  - SXI: ALV `C700` / 차트 `RGB(142,36,170)` (보라)
