...
- **렌더링 기술**: `CL_GUI_HTML_VIEWER` + HTML/CSS 막대차트.
  - (기존 IGS `CL_GUI_CHART_ENGINE` 은 대상 시스템에서 `Color`/`Bars` 커스터마이징이 반영되지 않아
    영역색·Top-N 가로막대를 보장할 수 없어 HTML 로 전환. O-9 IGS 자체는 가용하나 스타일 제약이 있음.)
...
- **색상 규약**: SM37 / ST22 / SXI 각각 고정 색상을 부여하여 차트·범례·ALV 행색 일관성 유지.
  - 차트 막대/제목 강조색은 HTML/CSS `#{hex}` 로 지정(ALV 행색과 동일 계열).
  - SM37: ALV `C400` / 차트 `#26A69A` (청록)
  - ST22: ALV `C600` / 차트 `#EF5350` (분홍/적)
  - SXI: ALV `C700` / 차트 `#FFA726` (주황)
