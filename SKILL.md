...
description: SAP GUI(SE38/SE80/SE24) 환경에서 ABAP 바이브 코딩을 위한 스킬. AI가 SAP에 직접 접속하지 못하고 사용자가 복사·붙여넣기로 코드를 옮기는 전제 하에, 요구사항 수집·스펙 확정·복붙용 코드 생성·활성화/테스트 안내를 수행할 때 사용한다. ALV 리포트, Module Pool, Function Module, Enhancement(BAdI/User-Exit), RFC/IDoc 인터페이스, Batch(BDC/BAPI), 서식(SmartForms/Adobe Forms) 작업에 적용된다.
license: MIT
---
...
- "ABAP 프로그램 만들어줘", "ALV 리포트 짜줘", "BAdI 구현해줘", "RFC 만들어줘"
- `requirements/` 템플릿 중 하나가 붙여넣어졌을 때
- `context/system-context.template.md` 가 제공되었을 때
- SAP 오류 메시지(`Syntax error`, `Dump`, `SY-SUBRC`) 해결을 요청할 때
...
| 시스템 | 사용 가능 문법 | 금지 문법 |
|---|---|---|
| ECC 6.0 / NW 7.31 이하 | 클래식 ABAP만 | 인라인 선언(`DATA(...)`), `VALUE #()`, `COND #()`, `FILTER #()` 금지 |
| NW 7.40 | 인라인 선언 일부 허용 | `FILTER`, 테이블 표현식 남용 금지 |
| NW 7.50 / S/4HANA | 모던 ABAP 허용 | 그래도 사용자가 "구문 보수적으로"를 고르면 클래식으로 폴백 |

> 릴리스를 모르면 **가장 보수적인(ECC 6.0 호환) 문법**으로 작성하고, 상단에 `"! ECC 6.0 호환 문법으로 작성됨"` 이라고 명시한다.
...
3. **한글 주석은 ASCII 안전하게**: SAP GUI 로그온 언어·코드페이지에 따라 한글이 깨질 수 있으므로, 핵심 주석은 한국어 + 영문 병기(`"! 고객별 매출 집계 / Sales total by customer`)를 권장하고, 시스템에 한글 입력이 불가하면 영문 주석판도 제공한다.
4. **활성화 순서 포함**: `SE11(DDIC) → SE38(프로그램) → SE93(T allocation) → SU21(권한)` 같은 순서를 코드 뒤에 체크리스트로 붙인다. 상세 양식은 `HARNESS.md` 5단계 참조.
...
## 4. 표준 워크플로우 (Harness 연동)

`HARNESS.md` 의 5단계 게이트를 따른다. 요약:

1. **Context 수집** — `context/system-context.template.md` + `requirements/00-common.md` + 해당 유형 템플릿(01~07)이 모두 채워졌는지 확인. 비어 있으면 코드 작성 금지, 템플릿 빈칸을 질문 리스트로 반환.
2. **Spec 확정** — 테이블·조인·선택화면·ALV 레이아웃·예외처리를 불릿 스펙으로 먼저 확정. 사용자 "OK" 전에는 코드 생성 금지(단, 사용자가 "바로 코드"를 요청하면 스펙+코드를 함께 제공).
3. **Code 생성** — Output Contract(3.3) 준수. 릴리스 게이트(3.2) 준수.
4. **Verify 안내** — 활성화 체크리스트 + 테스트 케이스 + 예상 덤프 대응표 제공.
5. **Handover** — SE38/SE80 복사 순서, T-code 생성(SE93), 권한(SU21/PFCG), 이송(TR) 요청서 초안까지 제공한다.
...
| `requirements/README.md` | 어떤 템플릿을 고를지 결정하는 라우터 |
| `requirements/00-common.md` ~ `07-forms.md` | 프로그램 유형별 1회 입력 요구사항 템플릿 |
...
> 릴리스를 모르면 ECC 6.0 호환으로 작성하겠습니다.

코드를 줄 때:

> 아래를 순서대로 SE38에 복사하십시오. ① `ZX..._TOP` → ② 메인. 활성화 후 §테스트 절차의 입력값으로 실행하십시오. 오류가 나면 메시지 번호 + `SY-SUBRC` + 덤프명(`ST22`)을 그대로 붙여넣어 주세요.
