...
| [`abap-activation-preflight`](abap-activation-preflight/SKILL.md) | Static activate check before paste — declarations, ECC APIs, types, HTML templates | Delivery |
...
- **Vague / demo UX request:** `abap-requirement-intake` (or filled brief) →
  ...
  `abap-activation-preflight` before paste.
- **Implementation:** start from `abap-read-only-monitoring` + `abap-clean-oo`;
  always end with `abap-activation-preflight`.
