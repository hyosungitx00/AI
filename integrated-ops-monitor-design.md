...
6. [화면 설계](#6-화면-설계)
   - 6.1 선택 화면 (SELECT-OPTIONS 타입 선언 주의 포함)
   - 6.2 대시보드 레이아웃
   - 6.6 Dynpro Screen 0100 PBO/PAI 설계
   - 6.7 ALV 이벤트 핸들러 설계
7. [처리 로직 설계](#7-처리-로직-설계)
8. [권한 및 보안 설계](#8-권한-및-보안-설계)
9. [성능 설계](#9-성능-설계)
10. [예외 및 에러 처리](#10-예외-및-에러-처리)
    - 10.2 커스텀 예외 클래스 (`ZCX_MON_ERROR`)
    - 10.3 메시지 클래스 번호 목록
11. [개발 표준 및 오브젝트 목록](#11-개발-표준-및-오브젝트-목록)
    - 11.3 DDIC 오브젝트 상세 정의 (구조체/테이블 타입 필드 목록)
    - 11.4 리포트 프로그램 골격 (`Z_OPS_MONITOR`)
    - 11.5 인터페이스 및 클래스 메서드 시그니처
    - 11.6 `ZCL_MON_NAVIGATOR` 메서드 시그니처
...
| `SO_JOB` | Select-Option (FOR `tbtco-jobname`) | - | 잡명 필터(SM37) |
| `SO_USER` | Select-Option (FOR `tbtco-sdluname`) | - | 사용자 필터(SM37/ST22) |
| `P_MAND` | MANDT | `SY-MANDT` | 클라이언트 필터(ST22/SXI) |
| `SO_IFACE` | Select-Option (FOR `sxmspemas-ob_name`) | - | 인터페이스명 필터(SXI) |
| `P_TOPN` | I | 5 | 차트 Top-N 개수(영역별 집중도 차트) |
| `P_MAXROW` | I | 250 | 영역별 ALV 최대 표시 행수(O-8). 최신순 제한, 초과 시 안내 |

> ⚠️ **SELECT-OPTIONS 타입 선언 주의**: `SELECT-OPTIONS` 구문의 `FOR` 절에는 **DDIC 데이터베이스 테이블 필드** 또는 **단순 데이터 오브젝트(`DATA` 변수)**를 지정해야 한다. 인터널 테이블 타입(`ZMON_T_*` 등)을 직접 참조하면 **문법 오류**가 발생한다.
>
> ```abap
> " ✅ 올바른 선언 — DDIC 테이블 필드 직접 참조
> SELECT-OPTIONS so_job   FOR tbtco-jobname.
> SELECT-OPTIONS so_user  FOR tbtco-sdluname.
> SELECT-OPTIONS so_iface FOR sxmspemas-ob_name.
>
> " ❌ 잘못된 선언 — 인터널 테이블 타입 참조 시 문법 오류
> " DATA lt_batch TYPE zmon_t_batch.
> " SELECT-OPTIONS so_job FOR lt_batch-jobname.   " SYNTAX ERROR
> ```
...
### 6.6 Dynpro 화면 흐름 설계 (Screen 0100)

#### 6.6.1 화면 구성 요소

```
Screen 0100 (대시보드)
├── Custom Control: CC_MAIN   ← CL_GUI_DOCKING_CONTAINER 배치 기반
│   └── CL_GUI_SPLITTER_CONTAINER (행 3단)
│       ├── Row 1 (고정 높이 ~40px): 요약 텍스트 출력 영역
│       │   └── CL_GUI_TEXTEDIT 또는 CL_DD_FORM_ELEMENT (신호등 + 건수)
│       ├── Row 2 (조정 가능 ~200px): 차트 패널
│       │   ├── 관점 드롭다운 (CL_GUI_COMBOBOX 또는 ALV 툴바 버튼)
│       │   └── CL_GUI_CHART_ENGINE
│       └── Row 3 (나머지 공간): 3분할 ALV 영역
│           └── CL_GUI_SPLITTER_CONTAINER (열 3단)
│               ├── Col 1: ALV Grid #1 (SM37)
│               ├── Col 2: ALV Grid #2 (ST22)
│               └── Col 3: ALV Grid #3 (SXI)
```
...
| 모듈명 | 종류 | 역할 |
|--------|------|------|
| `pbo_0100` | PBO (PROCESS BEFORE OUTPUT) | 컨트롤러 `display()` 최초 1회 호출, GUI 상태 설정 |
| `pai_0100` | PAI (PROCESS AFTER INPUT) | `OK_CODE` 처리 (`BACK`/`EXIT`/`CANCEL`/기능 키) |

```abap
" Screen 0100 Flow Logic 골격
...
MODULE pbo_0100 OUTPUT.
  SET PF-STATUS 'ZOPSMON_0100'.    " GUI 상태(툴바+메뉴)
  SET TITLEBAR  'ZOPSMON_TITLE'.   " 화면 타이틀
  IF go_dashboard IS NOT BOUND.
    go_dashboard = NEW zcl_mon_ui_dashboard( ).
    go_dashboard->create_controls( ).   " Splitter/ALV/차트 컨트롤 생성
    go_controller->call_display( io_dashboard = go_dashboard ).
  ENDIF.
ENDMODULE.

MODULE pai_0100 INPUT.
  DATA lv_ok TYPE sy-ucomm.
  lv_ok = ok_code. CLEAR ok_code.
  CASE lv_ok.
    WHEN 'BACK' OR 'EXIT' OR 'CANCEL'.
      go_dashboard->free_controls( ).
      LEAVE TO SCREEN 0.
    WHEN 'REFRESH'.
      go_controller->run( ... ).   " 재조회
    WHEN 'PERSP'.                  " 관점 토글
...
#### 6.6.3 GUI 상태(PF-STATUS) 정의 — `ZOPSMON_0100`

| 기능 키 | OK-CODE | 설명 |
|---------|---------|------|
| F3 / Back 버튼 | `BACK` | 이전 화면으로 복귀 |
| Shift+F3 / Exit | `EXIT` | 세션 종료 |
| F12 / Cancel | `CANCEL` | 취소 |
| Ctrl+F5 / Refresh | `REFRESH` | 현재 조건으로 재조회 |
| 드롭다운/버튼 | `PERSP` | 차트 관점 전환(Top-N ↔ 시간대별 추이) |

---

### 6.7 ALV 이벤트 핸들러 설계

ALV 더블클릭 이벤트를 처리하기 위해 **로컬 이벤트 핸들러 클래스**를 `ZCL_MON_UI_DASHBOARD` 내부에 정의한다.

```abap
" ZCL_MON_UI_DASHBOARD 내부 — 로컬 핸들러 (또는 인스턴스 메서드 이벤트 등록)
CLASS lcl_alv_handler DEFINITION.
  PUBLIC SECTION.
    DATA mo_nav    TYPE REF TO zcl_mon_navigator.
    DATA mv_area   TYPE char10.          " 'BATCH' / 'DUMP' / 'IFACE'

    " ALV DOUBLE_CLICK 이벤트 핸들러
...
CLASS lcl_alv_handler IMPLEMENTATION.
  METHOD on_double_click.
    DATA ls_row_batch TYPE zmon_s_batch.
    DATA ls_row_dump  TYPE zmon_s_dump.
    DATA ls_row_iface TYPE zmon_s_iface.

    CASE mv_area.
      WHEN 'BATCH'.
        sender->get_selected_rows( IMPORTING et_row_no = DATA(lt_rows) ).
        READ TABLE mt_batch INTO ls_row_batch INDEX e_row-index.
        IF sy-subrc = 0.
          zcl_mon_navigator=>navigate_batch(
            iv_jobname  = ls_row_batch-jobname
            iv_jobcount = ls_row_batch-jobcount ).
        ENDIF.
      WHEN 'DUMP'.
        READ TABLE mt_dump INTO ls_row_dump INDEX e_row-index.
        IF sy-subrc = 0.
          zcl_mon_navigator=>navigate_dump(
            iv_datum = ls_row_dump-datum
            iv_uzeit = ls_row_dump-uzeit
            iv_uname = ls_row_dump-uname ).
        ENDIF.
      WHEN 'IFACE'.
        READ TABLE mt_iface INTO ls_row_iface INDEX e_row-index.
        IF sy-subrc = 0.
          zcl_mon_navigator=>navigate_iface(
            iv_msgguid = ls_row_iface-msgguid ).
        ENDIF.
    ENDCASE.
  ENDMETHOD.
ENDCLASS.
...
> **이벤트 등록**: ALV Grid 인스턴스 생성 후 `SET HANDLER lo_handler->on_double_click FOR lo_alv_grid.` 로 연결한다. 영역별로 핸들러 인스턴스를 각각 생성하고 `mv_area`를 설정한다.
...
```abap
CLASS zcx_mon_error DEFINITION PUBLIC
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING mv_msgid TYPE sy-msgid  DEFAULT 'ZOPSMON'
                mv_msgno TYPE sy-msgno  DEFAULT '000'
                mv_msgv1 TYPE sy-msgv1  OPTIONAL.
ENDCLASS.
```

### 10.3 메시지 클래스 (`ZOPSMON`) 번호 목록(초안)

| 번호 | 타입 | 텍스트(한국어) | 비고 |
|------|------|----------------|------|
| 001 | E | 조회 시작일시가 종료일시보다 큽니다 | 기간 역전 |
| 002 | W | &1 영역 조회 권한이 없습니다 — 스킵됩니다 | `&1` = 영역명 |
| 003 | I | &1 영역: 에러 없음 (조회 기간 내 정상) | `&1` = 영역명 |
| 004 | W | &1 영역: 전체 &2건 중 상위 &3건만 표시됩니다 | 표시 상한 초과 안내 |
| 005 | E | &1 영역 조회 실패: &2 | FM/SELECT 오류 |
| 006 | I | 조회가 완료되었습니다 (총 &1건) | 조회 완료 |
...
### 11.3 DDIC 오브젝트 상세 정의

> 아래 구조체 및 테이블 타입은 ABAP Dictionary(SE11)에서 생성한다. SELECT-OPTIONS의 `FOR` 절이나 `TYPE` 지정 시 인터널 테이블 타입 대신 이 구조체의 **단일 라인(structure)** 또는 **DDIC 테이블 필드**를 참조한다.

#### 11.3.1 `ZMON_S_BATCH` — SM37 배치 에러 출력 구조

| 필드명 | 데이터 타입(참조) | 길이 | 설명 |
|--------|-----------------|------|------|
| `ICON` | `ICON` (데이터 엘리먼트) | 4 | 신호등 아이콘 (C 4자리, ALV 아이콘 컬럼) |
| `JOBNAME` | `TBTCO-JOBNAME` | 32 | 잡 이름 |
| `JOBCOUNT` | `TBTCO-JOBCOUNT` | 8 | 잡 카운트(키) |
| `STATUS` | `TBTCO-STATUS` | 1 | 상태 코드 |
| `STATUS_TX` | `CHAR30` | 30 | 상태 텍스트(산출) |
| `PROGNAME` | `TBTCP-PROGNAME` | 40 | 실행 프로그램(대표 스텝) |
| `SDLUNAME` | `TBTCO-SDLUNAME` | 12 | 스케줄/실행 사용자 |
| `STRTDATE` | `TBTCO-STRTDATE` | 8(D) | 시작 일자 |
| `STRTTIME` | `TBTCO-STRTTIME` | 6(T) | 시작 시간 |
| `ENDDATE` | `TBTCO-ENDDATE` | 8(D) | 종료 일자 |
| `ENDTIME` | `TBTCO-ENDTIME` | 6(T) | 종료 시간 |

#### 11.3.2 `ZMON_S_DUMP` — ST22 덤프 출력 구조

| 필드명 | 데이터 타입(참조) | 길이 | 설명 |
|--------|-----------------|------|------|
| `ICON` | `ICON` | 4 | 신호등 아이콘 |
| `DATUM` | `SYDATUM` (또는 `D`) | 8(D) | 발생 일자 (`RSDUMPTAB-SYDATE`) |
| `UZEIT` | `SYUZEIT` (또는 `T`) | 6(T) | 발생 시간 (`RSDUMPTAB-SYTIME`) |
| `UNAME` | `SYUNAME` | 12 | 발생 사용자 (`RSDUMPTAB-SYUSER`) |
| `AHOST` | `CHAR40` | 40 | 애플리케이션 서버 (`RSDUMPTAB-SYHOST`) |
| `RT_ERROR` | `CHAR40` | 40 | 런타임 에러 유형 (`RSDUMPTAB-DUMPID`) — Top-N 키 |
| `PROGNAME` | `PROGNAME` | 40 | 발생 프로그램 (`RSDUMPTAB-PROGRAMNAME`) |
| `INCLUDE` | `PROGNAME` | 40 | 인클루드 (`RSDUMPTAB-INCLUDENAME`) |
| `LINE` | `INT4` | 4 | 소스 라인 (`RSDUMPTAB-LINENUMBER`) |

> `RSDUMPTAB`에 `MANDT` 컬럼 없음 → 클라이언트 컬럼 미포함.

#### 11.3.3 `ZMON_S_IFACE` — SXI 인터페이스 에러 출력 구조

| 필드명 | 데이터 타입(참조) | 길이 | 설명 |
|--------|-----------------|------|------|
| `ICON` | `ICON` | 4 | 신호등 아이콘 |
| `EXE_DATE` | `SYDATUM` (또는 `D`) | 8(D) | 실행(에러) 일자 (`EXETIMEST` UTC→로컬 환산) |
| `EXE_TIME` | `SYUZEIT` (또는 `T`) | 6(T) | 실행(에러) 시간 |
| `IF_NAME` | `SXMSPEMAS-OB_NAME` | 60 | 인터페이스명(송신) — Top-N 키 |
| `IF_NS` | `SXMSPEMAS-OB_NS` | 120 | 네임스페이스(기본 숨김) |
| `OPERATION` | `SXMSPEMAS-OB_OPERATION` | 60 | 서비스 오퍼레이션 |
| `SENDER` | `SXMSPEMAS-OB_SYSTEM` | 50 | 송신 시스템 |
| `RECEIVER` | `SXMSPEMAS-IB_SYSTEM` | 50 | 수신 시스템 |
| `MSGSTATE` | `SXMSPMAST-MSGSTATE` | 1 | 메시지 상태 코드 |
| `STATE_TX` | `CHAR30` | 30 | 상태 텍스트(산출) |
| `ERRSTAT` | `SXMSPERROR-ERRSTAT` | 1 | 에러 상태/카테고리 |
| `MSGGUID` | `SXMSPMAST-MSGGUID` | 32 | 메시지 GUID(드릴다운 키, 숨김 가능) |

#### 11.3.4 `ZMON_S_SUMMARY` — 요약 영역 출력 구조

| 필드명 | 데이터 타입(참조) | 설명 |
|--------|-----------------|------|
| `AREA_CODE` | `CHAR10` | 영역 코드 (BATCH / DUMP / IFACE) |
| `AREA_TEXT` | `CHAR50` | 영역명(다국어 텍스트 심볼) |
| `CNT_TOTAL` | `INT4` | 전체 에러 건수(기간 내 전건) |
| `CNT_DISPLAY` | `INT4` | ALV 표시 건수(P_MAXROW 제한) |
| `ICON` | `ICON` | 신호등 아이콘(3단계) |
| `ICON_TEXT` | `CHAR20` | 신호등 텍스트(녹색/황색/적색) |
| `AREA_ICON` | `ICON` | 영역 구분 아이콘 |

#### 11.3.5 `ZMON_S_CHART_ITEM` — 차트용 집계 항목 구조

| 필드명 | 데이터 타입 | 설명 |
|--------|-----------|------|
| `AREA_CODE` | `CHAR10` | 영역 코드 (BATCH / DUMP / IFACE) |
| `LABEL` | `CHAR60` | Top-N 레이블 (잡명 / 에러유형 / IF명) |
| `CNT` | `INT4` | 건수 |
| `BUCKET` | `CHAR20` | 시간 버킷 레이블 (시간대별 추이 관점) |

#### 11.3.6 테이블 타입 (`ZMON_T_*`)

| 타입명 | 라인 타입 | 키 종류 | 용도 |
|--------|----------|---------|------|
| `ZMON_T_BATCH` | `ZMON_S_BATCH` | Standard (기본) | SM37 결과 내부 테이블 |
| `ZMON_T_DUMP` | `ZMON_S_DUMP` | Standard (기본) | ST22 결과 내부 테이블 |
| `ZMON_T_IFACE` | `ZMON_S_IFACE` | Standard (기본) | SXI 결과 내부 테이블 |
| `ZMON_T_SUMMARY` | `ZMON_S_SUMMARY` | Standard (기본) | 요약 영역 내부 테이블 |
| `ZMON_T_CHART` | `ZMON_S_CHART_ITEM` | Standard (기본) | 차트 집계 내부 테이블 |

> **코드 참조 예시** — 프로바이더 클래스 메서드 내에서 결과 테이블 사용:
> ```abap
> " ✅ 테이블 타입(ZMON_T_BATCH)을 내부 테이블 선언에 사용
> DATA lt_result TYPE zmon_t_batch.
> DATA ls_row    TYPE zmon_s_batch.   " 라인 구조는 ZMON_S_BATCH 참조
>
> " ✅ SELECT-OPTIONS는 DDIC 테이블 필드 참조로 선언 (6.1 참조)
> " SELECT-OPTIONS so_job FOR tbtco-jobname.
> " → so_job은 RANGES 타입으로 자동 생성됨 (SIGN/OPTION/LOW/HIGH)
> " → 조회 시: SELECT ... WHERE jobname IN so_job. 으로 사용
> ```
...
> 아래는 실행형 리포트의 선택화면 선언 및 이벤트 블록 구조 골격이다. SELECT-OPTIONS의 `FOR` 절은 **DDIC 테이블 필드**를 참조하도록 명시한다.

```abap
*&---------------------------------------------------------------------*
*& Report  Z_OPS_MONITOR
*& 통합 운영 모니터링 (SM37 / ST22 / SXI_MONITOR)
*&---------------------------------------------------------------------*
REPORT z_ops_monitor.
...
*----------------------------------------------------------------------*
TYPES:
  ty_so_job   TYPE RANGE OF tbtco-jobname,      " SELECT-OPTIONS 범위 타입
  ty_so_user  TYPE RANGE OF tbtco-sdluname,
  ty_so_iface TYPE RANGE OF sxmspemas-ob_name.

*----------------------------------------------------------------------*
* 선택 화면 (SELECTION SCREEN)
* ⚠ SELECT-OPTIONS는 반드시 DDIC 테이블 필드 또는 단순 DATA 변수 참조
*----------------------------------------------------------------------*
SELECTION-SCREEN BEGIN OF BLOCK b_period WITH FRAME TITLE TEXT-b01.
  PARAMETERS:
    p_frdat TYPE sy-datum,            " 조회 시작 일자
    p_frtim TYPE sy-uzeit,            " 조회 시작 시간
    p_todat TYPE sy-datum,            " 조회 종료 일자
    p_totim TYPE sy-uzeit,            " 조회 종료 시간
    p_hours TYPE i DEFAULT 24.        " 편의용 조회 시간(H)
SELECTION-SCREEN END OF BLOCK b_period.

SELECTION-SCREEN BEGIN OF BLOCK b_area WITH FRAME TITLE TEXT-b02.
  PARAMETERS:
    cb_sm37 TYPE c AS CHECKBOX DEFAULT 'X',   " SM37 영역 On/Off
    cb_st22 TYPE c AS CHECKBOX DEFAULT 'X',   " ST22 영역 On/Off
    cb_sxi  TYPE c AS CHECKBOX DEFAULT 'X'.   " SXI  영역 On/Off
SELECTION-SCREEN END OF BLOCK b_area.

SELECTION-SCREEN BEGIN OF BLOCK b_filter WITH FRAME TITLE TEXT-b03.
  " ✅ FOR 절에 DDIC 테이블 필드 직접 참조 — 인터널 테이블 타입 참조 금지
  SELECT-OPTIONS so_job   FOR tbtco-jobname.         " 잡명 필터
  SELECT-OPTIONS so_user  FOR tbtco-sdluname.        " 사용자 필터
  PARAMETERS:
    p_mand  TYPE mandt DEFAULT sy-mandt.             " 클라이언트
  SELECT-OPTIONS so_iface FOR sxmspemas-ob_name.     " 인터페이스명 필터
SELECTION-SCREEN END OF BLOCK b_filter.

SELECTION-SCREEN BEGIN OF BLOCK b_disp WITH FRAME TITLE TEXT-b04.
  PARAMETERS:
    p_maxrow TYPE i DEFAULT 250,      " 영역별 ALV 최대 표시 행수
    p_topn   TYPE i DEFAULT 5.        " 차트 Top-N 개수
SELECTION-SCREEN END OF BLOCK b_disp.

*----------------------------------------------------------------------*
* 전역 변수
*----------------------------------------------------------------------*
DATA: go_controller TYPE REF TO zcl_mon_controller.

*----------------------------------------------------------------------*
* INITIALIZATION
*----------------------------------------------------------------------*
INITIALIZATION.
  " 기본 조회기간 산정: 현재시각 −24H ~ 현재시각
  DATA(lv_to_ts) = cl_abap_tstmp=>utclong2tstmp(
    CONV utclong( sy-datum && sy-uzeit ) ).
  DATA(lv_fr_ts) = lv_to_ts - ( p_hours * 3600 ).
  cl_abap_tstmp=>split(
    EXPORTING tstmp    = lv_fr_ts
    IMPORTING datum    = p_frdat
              uzeit    = p_frtim ).
  p_todat = sy-datum.
  p_totim = sy-uzeit.
...
AT SELECTION-SCREEN.
  ...
  OR ( p_frdat = p_todat AND p_frtim > p_totim ).
    MESSAGE e001(zopsmon).   " 'FROM이 TO보다 큽니다'
  ENDIF.

AT SELECTION-SCREEN ON p_hours.
  " P_HOURS 변경 시 FROM/TO 재계산
  DATA(lv_new_fr) = cl_abap_tstmp=>utclong2tstmp(
    CONV utclong( sy-datum && sy-uzeit ) ) - ( p_hours * 3600 ).
  cl_abap_tstmp=>split(
    EXPORTING tstmp = lv_new_fr
    IMPORTING datum = p_frdat
              uzeit = p_frtim ).
...
START-OF-SELECTION.
  ...
  go_controller->run(
    iv_frdat   = p_frdat     iv_frtim   = p_frtim
    iv_todat   = p_todat     iv_totim   = p_totim
    iv_cb_sm37 = cb_sm37     iv_cb_st22 = cb_st22
    iv_cb_sxi  = cb_sxi
    it_so_job   = so_job[]   it_so_user  = so_user[]
    it_so_iface = so_iface[]
    iv_mand    = p_mand
    iv_maxrow  = p_maxrow    iv_topn    = p_topn ).
```

> `so_job[]`, `so_user[]`, `so_iface[]`는 `SELECT-OPTIONS`가 자동 생성하는 **내부 테이블**(SIGN/OPTION/LOW/HIGH 컬럼)이다. 메서드에 전달 시에는 `RANGES` 타입 또는 위에 선언한 `ty_so_*` 타입으로 매핑한다.

---

### 11.5 인터페이스 및 클래스 메서드 시그니처 상세

#### 11.5.1 `ZIF_MON_DATA_PROVIDER` (Interface)

```abap
INTERFACE zif_mon_data_provider PUBLIC.

  TYPES:
    BEGIN OF ty_area_info,
      area_code TYPE char10,
      area_text TYPE char50,
      area_icon TYPE icon,
    END OF ty_area_info,
    ...
    BEGIN OF ty_filter,
      ...
      totim    TYPE sy-uzeit,
      mand     TYPE mandt,
      so_job   TYPE RANGE OF tbtco-jobname,
      so_user  TYPE RANGE OF tbtco-sdluname,
      so_iface TYPE RANGE OF sxmspemas-ob_name,
      maxrow   TYPE i,
    END OF ty_filter.
  ...
  METHODS:
    " 영역 메타 정보 반환
    get_area_info
      RETURNING VALUE(rs_info) TYPE ty_area_info,

    " 에러 데이터 조회 (결과는 각 프로바이더가 내부에서 참조 형태로 보관)
    get_data
      IMPORTING is_filter    TYPE ty_filter
      EXPORTING ev_total_cnt TYPE i        " 기간 내 전체 건수(차트용)
                ev_disp_cnt  TYPE i        " ALV 표시 건수(P_MAXROW 제한)
      RAISING   zcx_mon_error,             " 조회 실패(권한 없음 포함)

    " Top-N 집계 키 반환 (Aggregator 위임용)
    get_topn_key_field
      RETURNING VALUE(rv_field) TYPE char30,

    " 결과 행을 공통 차트 집계 구조로 변환(Aggregator 위임용)
    get_chart_items
      RETURNING VALUE(rt_items) TYPE zmon_t_chart.

ENDINTERFACE.
```

#### 11.5.2 `ZCL_MON_CONTROLLER` (Class) — 주요 메서드

| 메서드 | 종류 | 파라미터(요약) | 설명 |
|--------|------|----------------|------|
| `constructor` | Instance | — | 프로바이더 레지스트리 초기화, 3개 프로바이더 등록 |
| `run` | Public | `iv_frdat/frtim/todat/totim`, `iv_cb_sm37/st22/sxi`, `it_so_job/user/iface`, `iv_mand/maxrow/topn` | 전체 조회 오케스트레이션 실행 |
| `register_provider` | Private | `io_provider TYPE REF TO zif_mon_data_provider` | 레지스트리에 프로바이더 추가 |
| `execute_providers` | Private | 필터 → 결과 취합 | 선택된 프로바이더만 `get_data` 호출 |
| `build_summary` | Private | 취합 결과 → `gt_summary` | 신호등 판정 포함 요약 생성 |
| `call_display` | Private | 결과 + 차트 → UI 클래스 위임 | 대시보드 화면 기동 |

#### 11.5.3 `ZCL_MON_DP_BATCH` — SM37 프로바이더 주요 로직

```abap
METHOD get_data.
  " 1. AUTHORITY-CHECK (S_BTCH_JOB, JOBACTION='LIST')
  AUTHORITY-CHECK OBJECT 'S_BTCH_JOB'
    ID 'JOBACTION' FIELD 'LIST'
    ID 'JOBGROUP' FIELD space.
  IF sy-subrc <> 0.
    RAISE EXCEPTION TYPE zcx_mon_error
      EXPORTING mv_msgid = 'ZOPSMON' mv_msgno = '002'.  " '권한 없음'
  ENDIF.

  " 2. TBTCO에서 STATUS='A' 조회
  "    기간 필터: ENDDATE/ENDTIME 복합 조건 (7.3 참조)
  SELECT jobname jobcount status strtdate strttime enddate endtime sdluname
    FROM tbtco
    INTO TABLE @DATA(lt_tbtco)
    WHERE status   = 'A'
      AND jobname IN @is_filter-so_job
      AND sdluname IN @is_filter-so_user
      AND (  " 복합 기간 조건 — build_datetime_range() 결과 WHERE 절 적용
           ( enddate = @is_filter-frdat AND endtime >= @is_filter-frtim )
        OR ( enddate > @is_filter-frdat AND enddate < @is_filter-todat )
        OR ( enddate = @is_filter-todat AND endtime <= @is_filter-totim )
           ).

  " 3. TBTCP LEFT JOIN 으로 대표 스텝(PROGNAME) 취득
  " 4. 결과를 ZMON_S_BATCH 구조로 변환 → mt_result (인스턴스 attribute)
  " 5. ev_total_cnt = LINES( lt_tbtco )
  "    최신순 정렬(ENDDATE/ENDTIME 내림차순) 후 P_MAXROW 제한
  "    ev_disp_cnt  = LINES( mt_result )
ENDMETHOD.
```

#### 11.5.4 `ZCL_MON_AGGREGATOR` — 집계 메서드

| 메서드 | 파라미터 | 설명 |
|--------|----------|------|
| `get_topn` | `it_items TYPE zmon_t_chart`, `iv_topn TYPE i`, `iv_area TYPE char10` → `rt_result TYPE zmon_t_chart` | 특정 영역의 Top-N 집계 반환 |
| `get_time_buckets` | `it_batch TYPE zmon_t_batch`, `it_dump TYPE zmon_t_dump`, `it_iface TYPE zmon_t_iface`, `iv_frdat`, `iv_todat` → `rt_result TYPE zmon_t_chart` | 적응형 버킷(≤24H=1H, ≤7일=1일) 시간 분포 반환 |
| `calc_bucket_size` | `iv_frdat`, `iv_todat` → `rv_minutes TYPE i` | 조회범위로부터 버킷 크기(분) 결정 |

---

### 11.6 `ZCL_MON_NAVIGATOR` — 드릴다운 메서드 시그니처

```abap
CLASS zcl_mon_navigator DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
    " SM37: 잡 로그 표시
    CLASS-METHODS navigate_batch
      IMPORTING iv_jobname  TYPE tbtco-jobname
                iv_jobcount TYPE tbtco-jobcount
      RAISING   zcx_mon_error.

    " ST22: 덤프 상세 화면 (CALL TRANSACTION 'ST22' — 표시 모드)
    CLASS-METHODS navigate_dump
      IMPORTING iv_datum TYPE sy-datum
                iv_uzeit TYPE sy-uzeit
                iv_uname TYPE sy-uname
      RAISING   zcx_mon_error.

    " SXI: 메시지 모니터 상세 (CALL TRANSACTION 'SXI_MONITOR' — 표시 모드)
    CLASS-METHODS navigate_iface
      IMPORTING iv_msgguid TYPE sxmspmast-msgguid
      RAISING   zcx_mon_error.

  PRIVATE SECTION.
    CLASS-METHODS assert_readonly.  " 변경성 호출 방지 내부 검증
ENDCLASS.
```
...
| v0.4 | 2026-06-22 | **SELECT-OPTIONS 타입 선언 수정** — `FOR` 절을 DDIC 테이블 필드 직접 참조로 변경(인터널 테이블 타입 참조 금지 명시). **추가 설계 베이스 보강** — ① DDIC 구조/타입 상세 필드 정의(11.3), ② 리포트 프로그램 골격 및 선택화면 코드(11.4), ③ `ZIF_MON_DATA_PROVIDER` 인터페이스·클래스 메서드 시그니처(11.5), ④ `ZCL_MON_NAVIGATOR` 시그니처(11.6), ⑤ Dynpro Screen 0100 PBO/PAI·GUI 상태 정의(6.6), ⑥ ALV 이벤트 핸들러 골격(6.7), ⑦ 예외 클래스·메시지 번호 목록(10.2/10.3) |
